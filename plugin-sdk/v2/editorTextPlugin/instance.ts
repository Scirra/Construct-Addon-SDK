...
PLUGIN_CLASS.Instance = class MyTextInstance extends SDK.IWorldInstanceBase
{
	...
	Draw(iRenderer: SDK.Gfx.IWebGLRenderer, iDrawParams: SDK.Gfx.IDrawParams)
	{
		...
		const webglText = this._webglText!;
		...
		webglText.SetSize(this._inst.GetWidth(), this._inst.GetHeight(), textZoom);
		...
		const font = this._inst.GetPropertyValue("font") as string;
		this.GetProject().EnsureFontLoaded(font);
		webglText.SetFontName(font);
		
		webglText.SetText(this._inst.GetPropertyValue("text") as string);
		...
		if (this._inst.GetAngle() === 0)
		{
			...
			iRenderer.Quad3(tempQuad, webglText.GetTexRect());
	...
	OnDoubleTap()
	{
		const text = this._inst.GetPropertyValue("text") as string;
