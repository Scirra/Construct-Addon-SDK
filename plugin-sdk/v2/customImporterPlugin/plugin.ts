...
async function HandleDataInMyFormat(droppedFileName: string, file: SDK.IZipFile | Blob, opts: SDK.UI.DragDropFileImportHandlerCallbackOpts)
{
	...
	const json = (await zipFile.ReadJson(entry)) as JSONObject;
	...
	for (const spriteData of (json["sprites"] as JSONArray))
	{
		promises.push(ImportSpriteData(zipFile, opts, spriteData as JSONObject));
	}
...
async function ImportSpriteData(zipFile: SDK.IZipFile, opts: SDK.UI.DragDropFileImportHandlerCallbackOpts, spriteData: JSONObject)
{
	...
	const filename = spriteData["file"] as string;
	const x = layoutX + (spriteData["offsetX"] as number);
	const y = layoutY + (spriteData["offsetY"] as number);
	const width = spriteData["width"] as number;
	const height = spriteData["height"] as number;
	const angle = (spriteData["angle"] as number) * (Math.PI / 180);	// convert to radians
